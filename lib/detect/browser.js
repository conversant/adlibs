...
var can = require('../canHas').can,
	...
	MAX_VER = {
		FIREFOX: 1000, // maximum versions are instituted to prevent bogus UAs from passing extremely large numbers
		CHROME: 1000,
		EDGE: 1000,
		OPERA: 1000
	},
...
var getChromiumVersion = function (win, uaVersion) {
	...
	if (has('Proxy', win)) {
		chromiumVersion = getVersion(uaVersion, 49, MAX_VER.CHROME);
...
var getGeckoVersion = function (win, uaVersion) {
	...
	if (has('PushManager', win)) {
		geckoVersion = getVersion(uaVersion, 44, MAX_VER.FIREFOX);
...
var getTridentVersion = function (win, uaVersion) {
	...
	if (can(d, 'pointerLockElement')) {
		tridentVersion = getVersion(uaVersion, 13, MAX_VER.EDGE);
		...
		if (looksLike(/Nintendo/i, ua)) {
			...
			if (can(has('opera', w), 'version')) {
				...
				if (detectedBrowser.version >= 28) { // chrome version is 28+ then it's on chrome's release cycle.
					detectedBrowser.version = getVersion(detectedBrowser.ua.version, detectedBrowser.version - 13, MAX_VER.OPERA); // Guess the version based on chrome's version.
				}
			}
		}
