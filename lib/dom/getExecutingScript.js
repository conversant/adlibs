 ...
 *
 * @param {{loadAttr, loadStarted}} [props]
 * @param {Function} [validatorFunc]
 * @param {HTMLScriptElement} [testScript] Used for IoC/testing.
 *
 * @returns {HTMLScriptElement|null}
 */
var getExecutingScript = function (props, validatorFunc, testScript) {
	...
	if (currentScriptEl && currentScriptEl.nodeName === 'SCRIPT' && validatorFunc(currentScriptEl) === true) {
		...
		for (i = scripts.length - 1; i >= 0; i--) {
			...
			if (!scriptEl.getAttribute(props.loadAttr) && validatorFunc(scriptEl) === true) {
	...
	if (outputScriptEl && outputScriptEl.nodeName === 'SCRIPT') {
		outputScriptEl.setAttribute(props.loadAttr, props.loadStarted);
	}
