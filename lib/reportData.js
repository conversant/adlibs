...
var objectToQueryString = function (params) {
	...
	for(paramName in params) {
		if (params.hasOwnProperty(paramName)) {
			paramList.push(encodeURIComponent(paramName) + '=' + encodeURIComponent(params[paramName]));
		}
	}
...
ReportData = function (baseURL, mp) {
	 ...
	 * @param {boolean} [isUnloadEvent] Optional boolean
	 * @param {string} [url] Optional runtime url if you were to want to call to a different base url
	...
	/**
	 * @instance
	 * Track an event by passing an object of key value pairs as added query string params to a
	 * @param {Object} params Object literal containing key value pairs to encode as query string params on the end of the url.
	 * @param {boolean} [isUnloadEvent] Optional boolean
	 * @param {string} [url] Optional runtime url if you were to want to call to a different base url
	 * @returns {string} Returns the path that was called via logging
