 ...
 * @returns {*}
 */
var sendReport = function (url, isUnloadEvent, isSendBeacon) {
    var wn = window.navigator;
    if (isSendBeacon === true) {
        sendBeacon(url, url.substring(url.indexOf('?') + 1).split('&'));
    }
    else {
...
ReportData = function (baseURL, mp) {
    ...
    var runQueue = function (isSendBeacon) {
        ...
        while (queryString) {
            sendReport(baseURL + queryString, isSendBeacon);
    ...
    self.log = function (params, isUnloadEvent, url, isSendBeacon) {
        ...
        if (!baseURL && !url) {
            ...
            return sendReport(trackURL, isUnloadEvent, isSendBeacon);
        }
    };
