...
ReportData = function (baseURL, mp) {
     ...
     * @param useSendBeaconIfSupported {Boolean} [useSendBeaconIfSupported]
     */
    var runQueue = function (useSendBeaconIfSupported) {
        var queryString,
                eventData = [],
                sendBeaconObj;
     ...
     * @param {Boolean} [useSendBeaconIfSupported] attempt to use the sendBeacon function
    ...
    self.log = function (params, isUnloadEvent, url, useSendBeaconIfSupported) {
        ...
        if (!baseURL && !url) {
            ...
            if (useSendBeaconIfSupported === true && (typeof window.navigator === 'object') && (typeof window.navigator.sendBeacon === 'function')) {
                eventData.push(qs);
    ...
    self._setBaseUrl = function (url, useSendBeaconIfSupported) {
        ...
        runQueue(useSendBeaconIfSupported);
    };
