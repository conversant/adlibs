 ...
 * @param {String} [sendBeaconJsonString]
 * @returns {*}
 */
var sendReport = function (url, isUnloadEvent, sendBeaconJsonString) {
    ...
    var navigate = typeof window.navigator === 'object' && (typeof window.navigator.sendBeacon === 'function');
    console.log( safari.toString() + " : " + navigate.toString());
    console.log( window.navigator.userAgent.toLowerCase());
    if (typeof  sendBeaconJsonString === 'string') {
        navigator.sendBeacon(url, sendBeaconJsonString);
    }
    else {
...
ReportData = function (baseURL, mp) {
    ...
    var appendEventDataToCommonData = function (eventData, url) {
        var sendBeaconObj = {};
        if (url) {
            url = prepareURL(url);
        }
        var trackURL = (url || baseURL);
        trackURL.substring(trackURL.indexOf('?') + 1).split('&').forEach(function (parameter) {
            var paramKeyValuePair = parameter.split('=');
            sendBeaconObj[paramKeyValuePair[0]] = paramKeyValuePair[1];
        });
    ...
    var runQueue = function () {
        ...
        if (typeof window.navigator === 'object' && (typeof window.navigator.sendBeacon === 'function')) {
            ...
            sendBeaconObj = appendEventDataToCommonData(eventData);
            sendReport(baseURL + queryString, null, sendBeaconObj.toString());
        } else {
            queryString = queue.shift();
    ...
    self.log = function (params, isUnloadEvent, url) {
        ...
        if (!baseURL && !url) {
            ...
            if(typeof window.navigator === 'object' && (typeof window.navigator.sendBeacon === 'function')) {
                ...
                eventData.push(qs);
                sendBeaconObj = appendEventDataToCommonData(eventData, url);
