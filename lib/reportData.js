 ...
 * @param url
 * @param isUnloadEvent
 * @param {String} [senBeaconJsonString]
 * @returns {*}
 */
var sendReport = function (url, isUnloadEvent, senBeaconJsonString) {
    var wn = window.navigator;
    if (typeof senBeaconJsonString === 'object') {
        sendBeacon(url, senBeaconJsonString);
    }
    else {
...
ReportData = function (baseURL, mp) {
     ...
     * @param {String} [senBeaconJsonString]
     */
    var runQueue = function (senBeaconJsonString) {
        var queryString = queue.shift();

        while (queryString) {
            sendReport(baseURL + queryString, senBeaconJsonString);
     ...
     */
    self.log = function (params, isUnloadEvent, url, senBeaconJsonString) {
        ...
        if (!baseURL && !url) {
            ...
            return sendReport(trackURL, isUnloadEvent, senBeaconJsonString);
        }
    };
     ...
     * @returns {string} returns the path that was called via logging
     */
    self.logWithElapsedTime = function (params, isUnloadEvent, url, senBeaconJsonString) {
        params.vtime = mp.now() - mp.startTime;
        return self.log(params, isUnloadEvent, url, senBeaconJsonString);
    };
     ...
     * @param {String} [senBeaconJsonString]
     */
    self._setBaseUrl = function (url, senBeaconJsonString) {
