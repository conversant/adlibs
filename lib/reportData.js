...
var sendReport = function (url, isUnloadEvent, sendBeaconJsonString) {
    ...
    if (typeof sendBeaconJsonString === 'object') {
        sendBeacon(url, sendBeaconJsonString);
    }
    else {
...
ReportData = function (baseURL, mp) {
     ...
     * @param {String} [sendBeaconJsonString]
     */
    var runQueue = function (sendBeaconJsonString) {
        var queryString = queue.shift();

        while (queryString) {
            sendReport(baseURL + queryString, sendBeaconJsonString);
     ...
     */
    self.log = function (params, isUnloadEvent, url, sendBeaconJsonString) {
        ...
        if (!baseURL && !url) {
            ...
            return sendReport(trackURL, isUnloadEvent, sendBeaconJsonString);
        }
    };
     ...
     * @returns {string} returns the path that was called via logging
     */
    self.logWithElapsedTime = function (params, isUnloadEvent, url, sendBeaconJsonString) {
        params.vtime = mp.now() - mp.startTime;
        return self.log(params, isUnloadEvent, url, sendBeaconJsonString);
    };
     ...
     * @param {String} [sendBeaconJsonString]
     */
    self._setBaseUrl = function (url, sendBeaconJsonString) {
