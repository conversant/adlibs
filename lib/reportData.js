 ...
 * @param {String} [jsonParamsString] Stringified json of query string params to be used if isSendBeacon is true
 * @returns {*}
 */
var sendReport = function (url, isUnloadEvent, isSendBeacon, jsonParamsString) {
    var wn = window.navigator;
    if (isSendBeacon === true) {
        sendBeacon(url, jsonParamsString);
    }
    else {
...
ReportData = function (baseURL, mp) {
     ...
     * @param jsonParamsString [jsonParamsString] Stringified json of query string params to be used if isSendBeacon is true
     */
    var runQueue = function (isSendBeacon, jsonParamsString) {
        var queryString = queue.shift();

        while (queryString) {
            sendReport(baseURL + queryString, isSendBeacon, jsonParamsString);
     ...
     */
    self.log = function (params, isUnloadEvent, url, isSendBeacon, jsonParamsString) {
        ...
        if (!baseURL && !url) {
            ...
            return sendReport(trackURL, isUnloadEvent, isSendBeacon, jsonParamsString);
        }
    };
     ...
     * @returns {string} returns the path that was called via logging
     */
    self.logWithElapsedTime = function (params, isUnloadEvent, url, isSendBeacon, jsonParamsString) {
        params.vtime = mp.now() - mp.startTime;
        return self.log(params, isUnloadEvent, url, isSendBeacon, jsonParamsString);
    };
     ...
     * @param {String} [jsonParamsString] Stringified json of query string params to be used if isSendBeacon is true
     */
    self._setBaseUrl = function (url, isSendBeacon, jsonParamsString) {
        ...
        runQueue(isSendBeacon, jsonParamsString);
    };
